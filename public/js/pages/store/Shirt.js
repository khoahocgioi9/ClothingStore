...
selectSortBy.onchange = function () {
	window.location.href = `/src/pages/store/Shirt.html?${this.name}=${this.value}`;
};
...
function loadMoreProductsShirt() {
	//TODO: get count shirt products from server
	if (start > productsShirt.length - 1) {
		showMoreBtn.remove();
		return;
	}

	//TODO: fetch paginated data from the server
	const products = productsShirt.slice(start, start + limit);
